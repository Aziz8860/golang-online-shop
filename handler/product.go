...
// c = context. Dengan bentuk gini, bisa nerima parameter
func ListProducts(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			return // setelah c.JSON harus return karena dia ngga menghentikan proses
		}
...
func GetProduct(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) { // ini cara membandingkan error di Golang
				log.Printf("Terjadi kesalahan saat mengambil data produk: %v\n", err)
